...
import { GuestInfo, fromJson } from "./WeddingDetails";
import { isRecord } from "./record";

type GuestListState = {
    guestList: unknown[] | undefined;
...
export class GuestList extends Component<GuestListProps, GuestListState> {
    constructor(props: GuestListProps) {
        super(props);
        this.state = {
            guestList: undefined, mollyGuestsTotal: undefined, mollyFamily: undefined,
            mollyExtra: undefined, jamesGuestsTotal: undefined, jamesFamily: undefined,
            jamesExtra: undefined
        }
    }
    render = (): JSX.Element => {
        if (this.state.guestList === undefined) {
    ...
    renderList = (code: JSX.Element[], i: number): JSX.Element => {
        ...
        if (code.length === this.state.guestList.length) {
            return <ul>{code}</ul>
        }
        const guest: GuestInfo = fromJson(this.state.guestList[i])
        if (guest.additional === undefined) {
            code.push(<li key={i}><a href="#" onClick={this.doOpenGuestDetailsClick}>{guest.guestName}</a> Guest of {guest.whichParty} +1?</li>)
        }
        else {
            code.push(<li key={i}><a href="#" onClick={this.doOpenGuestDetailsClick}>{guest.guestName}</a> Guest of {guest.whichParty} +
    ...
    doUpdateGuestCountChange = (i: number, mgt: number, mgf: number, mge: number,
        ...
        if (i === this.state.guestList.length) {
            this.setState({
            ...
            const g: GuestInfo = fromJson(this.state.guestList[i]);
            if (g.whichParty === "James") {
                const newtotal: number = (g.additional) ? jgt + 2 : jgt + 1;
                const newfam: number = (g.isFamily) ? jgf + 1 : jgf;
                ...
                const newfam: number = (g.isFamily) ? mgf + 1 : mgf;
    ...
    doFetchListJson = (value: unknown): void => {
        ...
        this.setState({ guestList: weddingGuests })
    }
}